...
out = load('1e5walks (d=0.001)');  % load the walks from other program
X = out(:,3);        %load column 3, distance from origin

dx = 0.005;           % bin size;

binedges = 0:dx:0.1;  % define evenly spaced array, start : step : end
...
xlim([0 0.1])                 % plot-range from x=0 to 1. 
grid on                     % adds major grid lines to current axes.
xlabel('delta R','FontSize',10)
